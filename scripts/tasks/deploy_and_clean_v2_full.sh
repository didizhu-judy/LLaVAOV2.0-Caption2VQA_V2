#!/bin/bash
# 一键：先部署 SGLang 模型，等 4 个实例全部健康后再跑清洗任务（v2、全量，不因 10 dirty 提前停）。
# 用法：
#   bash scripts/tasks/deploy_and_clean_v2_full.sh           # 前台跑清洗，日志 output/clean_v2_full.log
#   nohup bash scripts/tasks/deploy_and_clean_v2_full.sh &   # 整条链路后台跑
#
...
CLEAN_LOG="${CLEAN_LOG:-$ROOT/output/clean_v2_full.log}"
...
# ---------- 2. 跑清洗任务（全量，不设 STOP_AFTER_DIRTY） ----------
log "开始清洗任务（v2、全量，不因 dirty 数提前停），日志 $CLEAN_LOG"
...
export OPENBEE_V2=1
# 全量：不设置 STOP_AFTER_DIRTY，每个 jsonl 跑完为止
unset STOP_AFTER_DIRTY
...
echo "=== $(date -Iseconds) 启动清洗 v2（全量）===" >> "$CLEAN_LOG"
